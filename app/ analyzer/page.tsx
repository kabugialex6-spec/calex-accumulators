...
type BacktestTrade = { i: number; signal: 'buy' | 'sell'; reason: string; movePct: number; outcome: 'win' | 'loss' | 'flat' };
type BacktestSummary = { total: number; buy: number; sell: number; winRate: string; wins: number; losses: number; flats: number };

// ---------- Pure analysis functions (same rules as the standalone version) ----------
...
function computeSignalForData(data: Candle[]): SignalResult {
  ...
  const patternMatchesUp = pattern?.dir === 'bullish';
  const patternMatchesDown = pattern?.dir === 'bearish';

  let signal: 'buy' | 'sell' | 'wait' = 'wait';
  let reasonText = 'Conditions not aligned';
  const buyReady = trend.dir === 'up' && nearZone?.type === 'support' && patternMatchesUp;
  const sellReady = trend.dir === 'down' && nearZone?.type === 'resistance' && patternMatchesDown;
  if (buyReady && pattern) { signal = 'buy'; reasonText = `${pattern.name} at support, in uptrend`; }
  else if (sellReady && pattern) { signal = 'sell'; reasonText = `${pattern.name} at resistance, in downtrend`; }

  return { signal, reasonText, trend, nearZone, pattern };
}
...
export default function AnalyzerPage() {
  ...
  const [backtestMessage, setBacktestMessage] = useState<string | null>(
    'Connect and let some candles load first, then run the backtest. It replays the same rules candle-by-candle through history and checks what happened 10 candles after each signal.'
  );
  ...
  function runBacktest() {
    ...
    for (let i = 30; i <= maxI; i++) {
      ...
      if ((result.signal === 'buy' || result.signal === 'sell') && result.signal !== prevSig) {
        ...
        trades.push({ i, signal: result.signal, reason: result.reasonText, movePct: move * 100, outcome });
      }
      prevSig = result.signal;
    }

    if (!trades.length) {
      setBacktestMessage(`No signals fired across the ${candles.length} loaded candles. That's normal \u2014 this rule set is intentionally strict (needs trend + zone + pattern together). Try a different timeframe or let more history load.`);
      setBacktestSummary(null);
      setBacktestTrades(null);
      return;
    }
  ...
  return (
    <div className="analyzer-root" style={cssVars}>
      <div className="wrap">
        ...
        <p className="sub">Rule-based signal scanner on live Deriv price data \u2014 trend, zones, and candle patterns. Not a prediction. Verify every signal yourself before trading.</p>
        ...
        <div className="panel">
          ...
          <p className="note">Signal requires all three: trend direction, price at a support/resistance zone, and a confirming candlestick pattern. When a BUY or SELL first appears, you&apos;ll get a sound and a browser notification (if allowed).</p>
        </div>
        ...
        <div className="panel">
            ...
            <div className="bt-list">
              {backtestTrades.map((t, idx) => (
                <div className="bt-row" key={idx}>
                  <span className={`bt-tag ${t.signal}`}>{t.signal.toUpperCase()}</span>
                  <span>{t.reason}</span>
          ...
          {backtestTrades && (
            <p className="note">Each trade is scored on price movement 10 candles after the signal, vs a \u00b10.1% threshold. This is a rough approximation \u2014 no spread, slippage, or stop-loss placement is modeled.</p>
          )}
        </div>
      </div>
      ...
      <style jsx>{`
        ...
        .signal-badge { flex: 0 0 140px; border-radius: 8px; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 4px; padding: 16px 8px; transition: transform 0.15s; }
        ...
        .signal-badge .sub { font-size: 10px; color: var(--text2); text-transform: uppercase; }
        ...
        .bt-tag { padding: 2px 8px; border-radius: 4px; font-size: 10px; font-weight: 700; letter-spacing: 0.5px; }
        .bt-tag.buy { background: var(--buy-dim); color: var(--buy); }
        .bt-tag.sell { background: var(--sell-dim); color: var(--sell); }
        .bt-outcome { margin-left: auto; font-weight: 700; }
